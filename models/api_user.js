var UserModel = require('./model_users');
module.exports = {
  /**
	 * 添加数据
  ...
  find(data = {}, fields = null, options = {}) {
    console.log(data, fields, options);
    return new Promise((resolve, reject) => {
  ...
  findByIdAndUpdate(id, data) {
    console.log(id, data);
    return new Promise((resolve, reject) => {
      //model.findByIdAndUpdate(需要查找的id对象 ,callback)
      UserModel.findByIdAndUpdate(id, data, (error, doc) => {
        if (error) {
          reject(error)
        } else {
          resolve(doc)
